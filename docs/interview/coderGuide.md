...
**** CSS的重绘与回流?
  CSS 重绘与回流（重排）
什么是回流（Reflow/重排）
回流是指浏览器需要重新计算元素的位置和几何属性的过程。当页面布局或元素的几何属性发生变化时，就会触发回流。

触发回流的常见操作：
改变元素尺寸：width, height, padding, margin, border

改变元素位置：position, top, left, right, bottom

改变页面布局：display, float, flex, grid

DOM 结构变化：添加/删除元素，修改文本内容

窗口变化：窗口大小调整（resize 事件）

读取某些属性：offsetWidth, offsetHeight, scrollTop, clientWidth 等

什么是重绘（Repaint）
重绘是指当元素的样式发生变化但不影响布局时，浏览器需要重新绘制元素的外观。重绘不涉及几何位置计算。

触发重绘的常见操作：
改变颜色属性：color, background-color, border-color

改变背景：background-image, background-position

改变边框样式：border-style, border-radius

改变阴影：box-shadow, text-shadow

改变透明度：opacity

改变可见性：visibility（但注意 display: none 会触发回流）
